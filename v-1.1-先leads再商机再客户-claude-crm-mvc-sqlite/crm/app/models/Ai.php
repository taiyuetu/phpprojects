            ...
            'search_records' => [
                ...
                'params' => [
                    ...
                    'tables'  => ['label' => '范围', 'type' => 'table_list',
                                  'options' => ['lead', 'customer', 'deal', 'order', 'follow_up', 'activity', 'ai_request']],
            ...
            'get_record' => [
                ...
                'params' => [
                    'type' => ['label' => '类型', 'type' => 'enum', 'required' => true,
                               'options' => ['lead', 'customer', 'deal', 'order', 'follow_up', 'ai_request']],
...
TXT;
    ...
    public static function contextDigest(?int $userId = null, int $limit = 12): string
    {
        ...
        foreach (['customers' => '客户', 'leads' => '线索', 'deals' => '商机', 'orders' => '订单',
                  'follow_ups' => '跟进', 'ai_actions' => 'AI 记录'] as $tbl => $zh) {
            try {
    ...
    public static function mockCompletion(string $instruction, array $rounds = []): string
    {
        ...
        if (preg_match('~(删除|删掉|去掉)\s*(线索|客户|商机|订单|记录)\s*' . $ref . '~iu', $text, $d)) {
            $target = ['线索' => 'delete_lead', '客户' => 'delete_customer',
                       '商机' => 'delete_deal', '订单' => 'delete_order', '记录' => 'delete_ai_request'][$d[2]];
            $idKey = ['delete_lead' => 'lead_id', 'delete_customer' => 'customer_id', 'delete_deal' => 'deal_id',
                      'delete_order' => 'order_id', 'delete_ai_request' => 'action_id'][$target];
            $actions[] = ['tool' => $target,
    ...
    private static function checkItems($value, string $label): ?string
    {
        ...
        foreach (array_values($items) as $i => $item) {
            ...
            if (trim((string) ($item['product_name'] ?? '')) === '') {
                return $label . ' 第 ' . ($i + 1) . ' 行缺少 product_name';
            }
    ...
    private static function checkRecordRef(string $type, $value, int $userId, string $label): ?string
    {
        ...
        if (!$row) {
            ...
            $prefix = ['lead_id' => 'LEAD-', 'customer_id' => 'CUS-', 'deal_id' => 'DEAL-', 'order_id' => 'ORD-'][$type];
    ...
    {
        $class = ['lead' => 'Lead', 'customer' => 'Customer', 'deal' => 'Deal', 'order' => 'Order'][$type] ?? null;
        if ($class === null) {
    ...
    private static function querySurface(array $surface, string $term, int $limit, int $userId, array $filters = [], bool $whole = false): array
    {
        ...
        $select = implode(', ', array_unique(array_merge(['id'], $surface['show'], [$surface['owner']])));
        ...
        $out = [];
        foreach ($found as $row) {
            $owner = isset($row[$surface['owner']]) ? (int) $row[$surface['owner']] : null;
            ...
            $out[] = [
                ...
                'writable' => canManageResource($owner ?: null),
            ];
        }
        return $out;
    }
    ...
    public static function deleteImpact(string $tool, array $args): array
    {
        ...
        $table = ['lead' => 'leads', 'customer' => 'customers', 'deal' => 'deals',
                  'order' => 'orders', 'ai_request' => 'ai_actions'][$type];
        $row = (new Database())->query("SELECT * FROM {$table} WHERE id = {$id}")->single();
        $codeClass = ['lead' => 'Lead', 'customer' => 'Customer', 'deal' => 'Deal', 'order' => 'Order'][$type] ?? null;
    ...
    {
        $db = Database::connection();
        $table = ['lead' => 'leads', 'customer' => 'customers', 'deal' => 'deals', 'order' => 'orders'][$type];
        $label = ['lead' => '线索', 'customer' => '客户', 'deal' => '商机', 'order' => '订单'][$type];
        $row = (new Database())->query("SELECT * FROM {$table} WHERE id = {$id}")->single();
        if (!$row) {
            return ['ok' => false, 'message' => "{$label} #{$id} 已不存在（可能刚被删掉）"];
        }
        // 提示里一律用编号，和预览、搜索、回执说同一件事
        $codeClass = ['lead' => 'Lead', 'customer' => 'Customer', 'deal' => 'Deal', 'order' => 'Order'][$type];
        $code = (new $codeClass())->codeOf($row);
    ...
    public static function foundDigest(string $instruction, int $limit = 8): string
    {
        ...
        foreach ($words as $word) {
            foreach (['lead', 'customer', 'deal', 'order'] as $key) {
