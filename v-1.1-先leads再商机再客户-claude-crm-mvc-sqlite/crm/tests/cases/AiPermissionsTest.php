...
function test_destructive_tools_are_never_auto_executed(): void
{
    ...
    $tools = Ai::tools();
    assertEquals(5, count(Ai::destructiveTools()), '删除类工具就 5 个');
...
function test_the_tool_table_is_the_single_source_of_truth(): void
{
    ...
    assertEquals(21, count($tools), '工具数量变了要同步文档与提示词');
    $kinds = array_count_values(array_map(static fn($t) => $t['kind'], $tools));
    assertEquals(3, $kinds['read']);
    assertEquals(13, $kinds['write']);
    assertEquals(5, $kinds['delete']);
    ...
    assertTrue(textLength($prompt) < 6800, '系统提示仍受长度约束（实测 ' . textLength($prompt) . ' 字）');
