...
<div class="card card-table p-4">
    ...
    <div class="accordion" id="faqAccordion">
        ...
        <div class="accordion-item">
            <h2 class="accordion-header">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#faqO">
                    商品库是干什么的？为什么加明细时必须选商品？
                </button>
            </h2>
            <div id="faqO" class="accordion-collapse collapse" data-bs-parent="#faqAccordion">
                <div class="accordion-body">
                    以前每一行商品名都是手挨的：同一个东西被三个人写成 <code>6206 轴承</code>、
                    <code>深沟球轴承6206</code>、<code>bearing 6206</code>，于是销量算不准、报价对不上、
                    月底想知道“这个型号到底卖了多少”只能靠记忆。<strong>商品库</strong>（侧边栏「商品」，<code>/products</code>）
                    就是把这份目录固定下来：名称、SKU、规格、品牌、分类、单位、单价、参考价、状态。<br>
                    商机与订单的明细行现在都<strong>从商品库里选</strong>：
                    <ul class="mb-2">
                        <li>选择框是<strong>上下两层</strong>：上面输入框搜索（输 “6206” 立刻筛），
                            下面仍是原生 <code>select</code> 可以直接下拉点选 —— 两种习惯都顺手；
                            只有一个命中时自动选中，一个都没有时会提示“去新建商品”，不会静默留空。</li>
                        <li>选中后本行的名称 / SKU / 单位 / 单价自动带出；<strong>你在行里改过的不会被覆盖</strong>，
                            临时写一句“含安装调试”或给个折扣价都行。</li>
                        <li>明细里存的是<strong>成交快照</strong>：商品今天改价，不会改写昨天已经签出去的订单。</li>
                        <li>不想被强制选商品时，可在 设置 → 应用信息 把「明细必须选商品」关掉（默认开）。</li>
                    </ul>
                    两件要紧的事：
                    <ul class="mb-2">
                        <li><strong>升级前手填的行不会被弄坏。</strong>老明细原来没有商品可关联，页面上会标成
                            “历史手填”，不改就原样保留；一旦你改了它的名称或价格，就必须从商品库里选一个。</li>
                        <li><strong>被引用的商品删不掉。</strong>商品列表里点删除会把它改成「停用」：
                            以后选不到它，历史订单仍然看得懂自己卖过什么。要真删，先让订单不再引用它。</li>
                    </ul>
                    列表页顶部如果提示“N 条订单明细还没关联商品”，点<strong>收编为商品</strong>即可：
                    系统按（名称 + SKU）去重建成商品并把老行链回去，幂等、可反复点。
                    AI 助手同样只能引用商品库里的商品（<code>search_records(tables:product)</code> 查编号、
                    <code>create_product</code> / <code>update_product</code> / <code>delete_product</code> 管理目录），
                    跟人在页面上做的是同一套校验。
                </div>
            </div>
        </div>
