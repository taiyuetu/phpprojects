...
class Setting extends Model
{
    ...
    public static function definitions(): array
    {
        return [
            ...
            'items_require_product' => [
                'label'   => '明细必须选商品',
                'type'    => 'select',
                'default' => '1',
                'options' => [
                    ['value' => '1', 'label' => '必须从商品库选择（推荐）'],
                    ['value' => '0', 'label' => '允许继续手填商品名（旧做法）'],
                ],
                'hint'    => '控制商机与订单的商品明细：开启后每一行都必须从商品库里选一个商品'
                             . '（页面选择框与 AI 的 set_order_items 走同一套服务端校验）。'
                             . '升级前手填的历史行不改可原样保留，一改就必须选商品；'
                             . '商品库还没建全时可以临时关掉，不必因此卡住下单。',
            ],
            'items_require_product' => [
                'label'   => '明细必须选商品',
                'type'    => 'select',
                ...
                'hint'    => '管商机与订单的商品明细：开启后每一行都必须从商品库里选一个商品，'
                             . '页面选择框与 AI 的 set_order_items 走同一套服务端校验，不存在两套规矩。'
                             . '升级前手填的历史行不改可以原样保留，一改名称或价格就必须选商品。'
                             . '商品库还没建全时可临时关掉，不必因此卡住下单。',
            ],
            'currency_symbol' => [
                'label'   => '货币符号',
                'type'    => 'select',
            ...
            'ai_context_minutes' => [
                ...
                'default' => '60',
                'options' => [
                    ['value' => '0',     'label' => '关闭（每次都是全新对话）'],
                    ['value' => '15',    'label' => '最近 15 分钟'],
                    ['value' => '60',    'label' => '最近 1 小时（推荐）'],
                    ['value' => '240',   'label' => '最近 4 小时'],
                    ['value' => '1440',  'label' => '今天之内'],
                    ['value' => '4320',  'label' => '最近 3 天'],
                    ['value' => '10080', 'label' => '最近 7 天（最长）'],
                ],
                'hint'    => '每次提问时，把你这个账号在窗口内的历史请求（指令、他答了什么、状态、涉及哪些记录编号）'
                             . '作为 <history> 一并送进模型，于是“刚才那条线索”“上次那个印度客户后来怎么样了”能接得上。'
                             . '历史直接读审计表 ai_actions，不另存一份，所以不存在第二真相；窗口越长，提示词越长、响应越慢。',
            ],
            'ai_temperature' => [
                'group'   => 'ai',
