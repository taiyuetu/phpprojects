    ...
    <div class="row g-3 mt-2">
        ...
        <div class="col-md-3">
            <div class="border rounded p-3 text-center">
                ...
                <small class="text-muted">管理客户信息、商机、跟进记录</small>
            </div>
        </div>
        <div class="col-md-3">
            <div class="border rounded p-3 text-center">
                <i class="bi bi-clock-history fs-2 text-success"></i>
                <h6 class="mt-2">跟进记录</h6>
                <small class="text-muted">记录比价、无意向等跟进情况</small>
            </div>
        </div>
    </div>
</div>
...
<div class="card card-table p-4 mb-4">
    ...
    <p class="mb-3">系统的业务流程遵循"先线索，再商机，再客户"的原则：</p>
...
└─────────────────────────────────────────────────────────────────┘
   ...
   └───────────────┘                      └───────────────┘
    ...
    <ol class="mb-0">
        ...
        <li class="mb-0"><strong>推进成交</strong> — 在商机页面推进商机阶段（进行中 → 方案阶段 → 谈判中 → 成交）。</li>
    </ol>
</div>

<!-- 商机 vs 线索 vs 跟进记录 -->
<div class="card card-table p-4 mb-4">
    <h5 class="mb-3"><i class="bi bi-question-diamond me-2"></i>商机 vs 线索 vs 跟进记录</h5>
...
<div class="card card-table p-4 mb-4">
    ...
    <p>已流失的线索可以重新激活，点击操作栏的激活按钮 <i class="bi bi-arrow-counterclockwise text-success"></i>，线索将恢复为"已联系"状态，可继续跟进或转商机。</p>
</div>
...
<div class="card card-table p-4 mb-4">
    ...
    <p>每条跟进记录可以包含：标题、描述、下一步行动、下次跟进日期。</p>
</div>

<!-- 数据库迁移 -->
<div class="card card-table p-4 mb-4">
    <h5 class="mb-3"><i class="bi bi-database me-2"></i>数据库迁移</h5>
    <p>首次使用或升级时，需要执行以下SQL迁移文件：</p>
    <pre class="bg-light p-3 rounded small mb-3"><code># 1. 创建跟进记录表、leads关联客户
mysql -u root -p crm_db &lt; database/migrate_customer_detail.sql

# 2. 添加线索流失原因字段
mysql -u root -p crm_db &lt; database/migrate_lead_lost_reason.sql</code></pre>
</div>
